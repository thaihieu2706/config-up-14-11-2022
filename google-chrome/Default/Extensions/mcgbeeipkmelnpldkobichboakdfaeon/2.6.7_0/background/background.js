...
var notificationData = { "title": "Text Copied!", "subtitle": "You can paste (ctrl or cmd + V)", "body": "Save to History", "url": "https://useblackbox.io/history" }
var startDate
...
function injectProcessing(tab, status) {
	chrome.tabs.sendMessage(tab.id, { message: "processing" , status: status}, (res) => {
		if (res) {
			clearTimeout(timeout)
		}
	})
	...
	var timeout = setTimeout(() => {
		...
		setTimeout(() => {
			chrome.tabs.sendMessage(tab.id, { message: "processing" , status: status })
		}, 100)
	}, 100)
}

function injectNotification(tab, data) {
	chrome.tabs.sendMessage(tab.id, { message: "notification" , data: data}, (res) => {
		if (res) {
			clearTimeout(timeout)
		}
	})
	...
	var timeout = setTimeout(() => {
		...
		setTimeout(() => {
			chrome.tabs.sendMessage(tab.id, { message: "notification" , data: data })
		}, 100)
	}, 100)
}

function injectCopied(tab) {
	chrome.tabs.sendMessage(tab.id, { message: "copied", userId: userId, notificationData: notificationData }, (res) => {
		if (res) {
			clearTimeout(timeout)
		}
	})
	...
	var timeout = setTimeout(() => {
		...
		setTimeout(() => {
			chrome.tabs.sendMessage(tab.id, { message: "copied", userId: userId })
		}, 100)
	}, 100)
}
...
function capReach(){
	chrome.storage.sync.get(function (items) {
		...
		if (items.usage){
			fetch(site + `isCapped/${uid}/${items.usage}`).then(function (response) {
				response.json().then(function (data) {
					...
					if (data.message == "showcap"){
						...
						if (data.userStatus){
							if (data.userStatus == 'top'){
								resetDailyUsage()
								var capNumber = 9;
								if (data.capUsage){ capNumber = data.capUsage }
						...
						chrome.tabs.getSelected(null, (tab) => {//clear the crosshair
							// inject(tab)
							if (data.userStatus){
								if (data.userStatus == 'top'){
									...
									if (items.viewedCap == undefined){
										chrome.tabs.create({ url: data.url })
										chrome.storage.sync.set({ viewedCap: true })
									}
								}
							}
						})
...
function resetDailyUsage() {
	chrome.storage.sync.get(function (items) {
		if (items.usage == 5){
			...
			var secondsCap = (endDateCap.getTime() - startDateCap.getTime()) / 1000;
...
chrome.commands.onCommand.addListener((command) => {
	if (command === "take-screenshot") {
		...
		var uId
		chrome.storage.sync.get(["userId"], function (items) {
			if (items.userId == undefined) {redirectToLogin()}
			else {
				uId = items.userId;
				  	...
				  	if (JSON.parse(result).status==true){
				  		chrome.tabs.query(
				  			...
				  			function (tabs) {
				  				...
				  				injectProcessing(tabprocessing, "Video is processing")
...
async function processingvideo(videoId) {
	...
	fetch("https://blackboxapp.co/processingytvideo", requestOptions)
	  ...
	  .then(result => {
	  	...
	  	if (resultv.index){index=resultv.index}
	  })
	  .catch(error => console.log('error', error));
}
...
async function processvideo(videoId, search) {
	...
	fetch("https://blackboxapp.co/videoquery", requestOptions)
		...
		.then((result) => {
			...
			chrome.tabs.query({ currentWindow: true, active: true }, function (tabs) {
				if (resultArr.length!=0){ injectPlay(tabs[0], resultArr[0].join(";"), search, resultArr[1].join(";")) }
				else { injectPlay(tabs[0], "", search, "") }
			})
		})
		.catch((error) => console.log("error", error))
}
...
chrome.runtime.onMessage.addListener((req, sender, res) => {
	...
	} else if (req.message === "getvideoimglocal") {//getvideoimg request for local files
...
async function processFrame(data, imgName) {
	...
	fetch("http://34.66.142.207/getvideoimg", requestOptions)
	  ...
	  .then(result => {
	  	...
	  	if (imgName.includes('-1.jpg')==false){//not complete yet
	  		chrome.tabs.query(
					{ currentWindow: true, active: true },
					function (tabs) {
						if (tabs[0].url){
							if (tabs[0].url.includes('youtube') || tabs[0].url.includes('coursera')){
								chrome.tabs.sendMessage(tabs[0].id, {
								...
								chrome.tabs.query({}, function(tabs) { 
									...
									for (var k=0;k<allTabs.length;k++){
										if (allTabs[k].url){
											if (allTabs[k].url.includes('youtube') || allTabs[k].url.includes('cousera')){
