...
function addToaster(str = "Video processed", userId) {
		...
		<div style="cursor:pointer;" class="black-box-onboard-message bottom cookie-banner-wrapper---brix-copy cookie-banner-2---brix-copy preview-page-copy">
			...
			<img src="${chrome.extension.getURL(
				...
				<img src="${chrome.extension.getURL(
					...
					<p class="paragraph cookie-banner-2-paragraph---brix">${notificationData.title}<a href=""></a>
					</p>
				</div>
			...
			<div class="cookie-banner-2-right-content---brix">
				<a href="" style="font-size:14px" class="button-primary cookie-banner-button-2---brix w-button">${notificationData.body}</a>
				<img src="${chrome.extension.getURL(
	...
	$(".black-box-onboard-message-holder").on("click", function(){ window.open(notificationData.url) })
...
chrome.runtime.onMessage.addListener((req, sender, res) => {
	...
	if (req.message === "play") {
		...
		//Define arrayVisual
		if (arrayResults[0].includes('"x"')){//its a visual result
			arrayVisual = [[],[]];  // clear the results values for a new search
			if (isYt(window.location.href)){
				document.getElementsByClassName('blackbox-filter-visual')[0].style='color:#398499;cursor:pointer;';
				document.getElementsByClassName('blackbox-filter-audio')[0].style='color:white;cursor:pointer;';
			}
		}else{
		...
		var k = indexResult;
		if (arrayResults[k].includes('"x"')){//display visual results
		...
		if (window.location.href.includes('youtube')==false && window.location.href.includes('youtu.be')==false){
			const $el = $($("video")[0])
			$container = $("#player")
			$container = $el.parent()
	...
	if (req.message === "processing") {
		...
		} else if ( req.status == "Video Processed" || req.status == "video processing complete" ) {
			document.getElementsByName("searchfield")[0].placeholder = req.status
			document.getElementById("searchfield").focus()
			setTimeout(() => {
				document.getElementsByName("searchfield")[0].placeholder = "Search With Blackbox..."
			}, 3000)
		} else if (req.status == "video can't be processed") {
			if (window.location.href.includes('youtube') || window.location.href.includes('youtu.be')){
		...
		$(document).on("click", ".blackbox-filter-visual", function () {
			...
			document.getElementsByClassName('black-box-item-counter')[0].innerHTML=`...`
		})
		$(document).on("click", ".blackbox-filter-audio", function () {
			document.getElementsByClassName('blackbox-filter-visual')[0].style='color:white;cursor:pointer;';
			document.getElementsByClassName('blackbox-filter-audio')[0].style='color:#398499;cursor:pointer;';
	...
	if (req.message === "startVisual"){
		if (req.status =='Visual Processing Complete'){
			...
			video.currentTime = 0;
			processingOverlay();
			if (window.location.href.includes('http')){//live website processing
...
async function sendImg(sec) {
	...
	}else{ //there is an ad or loader wait 1sec
		setTimeout(function () {
			sendImg(video.currentTime); 
		}, 1000)
	}
...
async function sendImgLocal(sec) {// function for local image genration
	var videoId = getYoutubeId(window.location.href); 
	if (video.currentTime < video.duration){
...
function adShowing(){
	if (window.location.href.includes('youtube') || window.location.href.includes('youtu.be')){
		if (document.querySelector('div.ad-showing') || document.getElementsByClassName('ytp-spinner')[0].style.display != 'none'){
		    return true;
		}else{
		    return false
		}
...
function processImgLocal(imgName) {// function like processImg for local files
	chrome.runtime.sendMessage({
...
function videoDefine(url) {//function that takes the input=url-> output video element - for all players
	var video
...
async function processingOverlay(){
	var $newEl = $(` <div class="black-box-processing-text">Processing...</div> `)
	if (isYt(window.location.href)){
		if (document.getElementById('player-theater-container').innerHTML!=''){
...
function updateProcessing(sec) {
	document.getElementsByClassName('black-box-processing-text')[0].innerText=`Processing ${Math.round(sec*100/video.duration)}%`
}
